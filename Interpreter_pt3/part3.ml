...
let rec compile progs =
  match progs with 
  | (Name n) -> " Push " ^ n ^ " Lookup "
  | (Int n) -> " Push " ^ string_of_int(n) ^ " "
  | (Unit) -> " Push () "
  | (Fun ("_", arg, t)) -> " Fun _ " ^ arg ^ " " ^ compile t ^ " End " ^ compile (Name "_")
  | (Fun (fn, arg, t)) -> " Fun " ^ fn ^ " " ^ arg  ^ " " ^ compile t ^" End "^ compile (Name fn)
  | (Add (e1, e2)) -> compile e1 ^ compile e2 ^ " Add "
  | (Sub (e1, e2)) -> compile e1 ^ compile e2 ^ " Sub "
  | (Mul (e1, e2)) -> compile e1 ^ compile e2 ^ " Mul " 
  | (Div (e1, e2)) -> compile e1 ^ compile e2 ^ " Div "
  | (Trace e) -> compile e ^ " Trace "
  | (Ifgz (cond, e1, e2)) -> compile cond ^ " If Begin " ^ compile e1 ^ " End Else Begin " ^ compile e2 ^ " End End"
  | (LetIn (n, e1, e2)) -> "Begin Push " ^ n ^ " " ^ compile e1 ^ " Let " ^ compile e2 ^ " End "
  | (App (t1, t2)) -> compile t1 ^ compile t2 ^ " Call "

let test str =
  match parse_prog str with
  | Some(progs,[]) -> compile progs
  | _ -> "Error"
