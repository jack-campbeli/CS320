...
let rec eval (stack : stack) (output : output) (local : env) (global : env) (commands : commands) : (output * stack option) * (env * env) =
  ...
  | BeginEnd (cmd_lst) :: commands -> (
      ...
      | ((out, Some (h::t)),(local, global)) -> (eval (h::stack) (out@output) local global commands)
      | _ -> ((output, None),([],[])))
      ...
      (
        ...
        | VBool b :: rest -> (
            ...
            then (
              ...
              | ((out, Some (s)),(new_local, new_global)) -> 
                (eval (s) (out@output) (local@new_local) (global@new_global) commands)
              | _ -> ((output, None),([],[])))
            else  
              (
